...
def arithmetic_arranger(problems :list[str], show_answers=False):
    ...
    for i in problems:
        ...
        problem_sol = ''
        width = max(len(right),len(left)) + 2
        if middle == "+":
                top_row.append(left.rjust(width))
                mid_row.append(middle + right.rjust(width - 1))
                dash_row.append('-'*width)
                bottom_row.append(str(int(left)+int(right)).rjust(width))
...
print(arithmetic_arranger(['3 + 90','9000 + 3']))